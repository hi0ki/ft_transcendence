...
class SocketService {
    ...
    connect(): Promise<{ socketId: string; userId: number; email: string; username?: string }> {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.socket = io(SOCKET_URL, {
                ...
                // ← removed auth: { token } — cookie handles auth now
                reconnection: true,
    ...
    on(event: string, callback: Function) {
        ...
        if (this.listenerWrappers.get(event)!.has(callback)) return;

        const wrapper = (...args: any[]) => callback(...args);
        this.listenerWrappers.get(event)!.set(callback, wrapper);
        if (this.socket) this.socket.on(event, wrapper);
    }
